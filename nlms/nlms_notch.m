...
notch_output = filter(B, A, e);

% Compute SNR after Notch Filtering
snr_notch = 10*log10(sum(clean_speech.^2) / sum((notch_output - clean_speech).^2));

% Normalize signals
e = notch_output / max(abs(notch_output));

% Save as WAV files
audiowrite('cleaned_speech_nlms_notch.wav', e, fs);
audiowrite('nlms_output.wav', e, fs);

% Print SNR Report

fprintf('1. Original Noisy Speech SNR: %.2f dB\n', snr_original);
fprintf('2. After  NLMS Filtering SNR: %.2f dB (Improvement: %.2f dB)\n', snr_nlms, snr_nlms - snr_original);
fprintf('3. After Adaptive Notch Filtering SNR: %.2f dB (Improvement: %.2f dB)\n', snr_notch, snr_notch - snr_nlms);
fprintf('4. Total Improvement: %.2f dB\n', snr_notch - snr_original);
...
subplot(3,2,3); plot(t, e); title('After  NLMS Filtering'); xlabel('Time (s)'); ylabel('Amplitude');
subplot(3,2,4); fft_nlms = fft(e); plot(f, abs(fft_nlms(1:length(f)))./num_samples); title('Spectrum After NLMS'); xlabel('Frequency (Hz)');
subplot(3,2,5); plot(t, notch_output); title('After Adaptive Notch Filtering'); xlabel('Time (s)'); ylabel('Amplitude');
subplot(3,2,6); fft_notch = fft(notch_output); plot(f, abs(fft_notch(1:length(f)))./num_samples); title('Cleaned Speech Spectrum After Notch'); xlabel('Frequency (Hz)');

% Play cleaned speech
fprintf('Playing cleaned speech after  NLMS + Notch...\n');
sound(e, fs);